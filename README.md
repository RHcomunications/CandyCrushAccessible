...
- **v1.2.1 (August 24, 2026)**: Audrik x Narayan Formula (3D Room Reverb & Dynamic Ducking) — integrated subtle DX8 room reverb (`-12 dB`, `450 ms`) to create a natural acoustic room soundstage placing the board in front of the player, and transparent music volume sliding (`BASS_ChannelSlideAttribute`) to `35%` in `200 ms` during announcer voice events with a `1000 ms` smooth restoration, preserving 100% pure audio dynamics with zero digital clipping.
- **v1.2.0 (August 24, 2026)**: Complete Audio Fidelity & Sound Mapping Overhaul — fixed lose track mapping in `MusicMap.cs` (eliminated upbeat level start fanfare `candy_crush_intro2` on defeat, stopping music immediately and playing only authentic defeat SFX `level_failed1.wav`), re-anchored `ingredient`, `ingredient2`, and `nut` to `nut_out1.wav` (authentic King sound for collecting nuts/cherries), and restored faithful fallback mappings for level results, combo pitch scale, special candy creations, and destructive explosions in `AudioMap.cs`.
- **v1.1.9 (August 19, 2026)**: DEBUG-only dev panel — new "Max everything (dev only)" option at the end of the Options menu, guarded with `#if DEBUG` (grants 9999 gold bars, 99999 coins, 99 lives, 50 of each booster, all levels unlocked and resets the daily bonus). Verified at binary level that the max-out method does NOT exist in the Release DLL (zero production leaks). Daily bonus is now claimed exclusively in the shop with Enter (fixed the silent auto-claim on startup) so the full flow can be verified.
- **v1.1.8 (August 19, 2026)**: Total balance clarity — a single complete sentence ("You have X gold bars and Y coins") replaces the ambiguous separate "Lingotes: X. Monedas: Y." phrases. Balance is always visible and announced in the main menu, shop and booster selection. "X of 3" stars format on the level map ("Stars: 2 of 3") replaces the meaningless "0". Clear pack purchase messages ("Purchased! You received 10 gold bars").
- **v1.1.7 (August 19, 2026)**: Object-oriented 3D binaural audio (Dolby/Atmos principle): each sound is modeled as a 3D object in front of a fixed listener with equal-power azimuth panning, Dolby-style distance roll-off and depth-based pitch, preserving per-candy signature tones. Clear coins vs gold bars separation in the shop: a single complete balance sentence ("You have X gold bars and Y coins"), per-currency section headers, explicit prices and a balance always visible in the main menu, shop and booster selection. "X of 3" stars on the level map and clear pack purchase messages ("You received 10 gold bars").
...
- **v1.2.1 (24 de Agosto de 2026)**: Fórmula Audrik x Narayan (Reverb 3D y Ducking Dinámico) — integración de reverberación de cuarto sutil (`BASS_FX_DX8_REVERB` a `-12 dB`, `450 ms`) para envolver el tablero frente al oyente y atenuación musical suave por deslizamiento (`BASS_ChannelSlideAttribute` al `35%` en `200 ms`) durante las locuciones del narrador, con `1000 ms` de recuperación, garantizando 100% de pureza acústica y cero saturación digital.
- **v1.2.0 (24 de Agosto de 2026)**: Corrección integral de fidelidad sonora y mapeo de audio — corrección de pista de derrota en `MusicMap.cs` (eliminada la fanfarria alegre de intro `candy_crush_intro2` al perder, deteniendo la música y reproduciendo solo el SFX auténtico `level_failed1.wav`), reubicación de `ingredient`, `ingredient2` y `nut` a `nut_out1.wav` (sonido original de King de 2012 para recolectar ingredientes), y restauración de respaldos (fallbacks) fidedignos para resultados de nivel, escala tonal de combos, creación de especiales y explosiones en `AudioMap.cs`.
- **v1.1.9 (19 de Agosto de 2026)**: Panel de desarrollo exclusivo DEBUG — nueva opción "Maxear todo (solo desarrollo)" al final del menú de Opciones, bloqueada con `#if DEBUG` (concede 9999 lingotes, 99999 monedas, 99 vidas, 50 de cada potenciador, todos los niveles desbloqueados y reinicia el bono diario). Verificado a nivel binario que el método de maxización NO existe en la DLL de Release (0 fugas de poderes a producción). El bono diario ahora se reclama exclusivamente en la tienda con Enter (corregido el auto-reclamo silencioso al arrancar) permitiendo verificar el flujo completo.
- **v1.1.8 (19 de Agosto de 2026)**: Claridad total del saldo — una única frase completa ("Tienes X lingotes de oro y Y monedas") sustituye las frases sueltas y ambiguas "Lingotes: X. Monedas: Y.". El saldo siempre está visible y se anuncia en el menú principal, la tienda y la selección de potenciadores. Formato "X de 3" en las estrellas del mapa ("Estrellas: 2 de 3") en lugar del "0" sin utilidad. Mensajes de compra de paquetes claros ("¡Comprado! Recibiste 10 lingotes de oro").
- **v1.1.7 (19 de Agosto de 2026)**: Audio binaural 3D orientado a objetos (principio Dolby/Atmos): cada sonido se modela como un objeto 3D frente a un oyente fijo con paneo por acimut igual-potencia, atenuación por distancia tipo Dolby y tono por profundidad, conservando la firma tonal de cada caramelo. Separación clara de monedas y lingotes en la tienda: saldo en una única frase completa ("Tienes X lingotes de oro y Y monedas"), encabezados de sección por divisa, precios explícitos y saldo siempre visible en menú principal, tienda y selección de potenciadores. Estrellas "X de 3" en el mapa y mensajes de compra claros ("Recibiste 10 lingotes de oro").
